resource google_project_service "google_run_blog_service" {
...
resource "google_cloud_run_service" "spring_boot_terraform_sql_cloud_run_service" {
  name     = "spring-boot-blog-service"
  location = "us-central1"

  template {
    spec {
      containers {
        image = "gcr.io/ultimate-hydra-288114/spring-boot-sample-blog-terraform-sql-gcloud-run"
        env {
  ...
  depends_on = [
  google_project_service.google_run_blog_service]
